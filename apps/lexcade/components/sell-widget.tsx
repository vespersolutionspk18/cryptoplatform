  ...
  return (
    <>
      ...
      <Button 
        ...
        onClick={handleSell}
      >
        {isProcessing ? 'Processing...' : `Sell ${cryptoSymbol}`}
      </Button>
    </>
  );
});
...
const UsdDisplay = ({ cryptoAmount }: { cryptoAmount: number }) => {
  const { cryptoConfig } = useCrypto();
  const { price } = usePriceUpdates(cryptoConfig.binanceSymbol);
  const usdAmount = cryptoAmount * price;
  ...
  return (
    <>
      ...
      <Card className="p-4 gap-0 rounded-[0.75rem]">
        <SellSummary cryptoAmount={cryptoAmount} cryptoSymbol={cryptoConfig.symbol} onReset={handleReset} />
...
SellWidget.displayName = 'SellWidget';